...
class _EditTaskScreenState extends ConsumerState<EditTaskScreen> {
  ...
  void _initializeTaskData(TaskModel task) {
  ...
  Future<void> _submit(TaskModel originalTask) async {
    ...
    try {
      ...
      if (mounted) {
        _showSnackBar('Task updated successfully!', isError: false);
        context.pop();
      }
    } catch (e) {
      if (mounted) {
      ...
      body: SafeArea(
        child: taskAsync.when(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: ChoiceChip(
                            ...
                            selected: _selectedStatus == TaskStatus.pending,
                            onSelected: isLoading
                                ? null
                                : (selected) {
                                    if (selected) {
                    ...
                    // Save Button
                    ElevatedButton(
                      onPressed: isLoading ? null : () => _submit(task),
                      child: isLoading
                          ? const SizedBox(
                              height: 24,
                              width: 24,
          ...
          error: (error, _) => Center(
            child: Text('Error loading task details: $error'),
          ),
        ),
      ),
    );
